...
<body>
    ...
    <div class="container-scroller">
        ...
        <script>
            ...
            var pusher = new Pusher('6bf2a502de619b358656', {
                cluster: 'ap2',
                forceTLS: false,
            });
            if (!window.isEventBound) {
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                if (!window.isEventBound) {
                    var channel = pusher.subscribe('private-one-to-one-message');
                    channel.bind('testingEvent', function(data) {
                        console.log('Received event data:', data);
                        if (data && data.message) {
                            alert("Received message: " + data.message); // Show message if it exists
                        } else {
                            console.log('No message data received');
                        }
                    });
                    window.isEventBound = true;
                }
            })
        </script> --}}
        {{-- private end  --}}
            ...
            $(document).ready(function() {
                ...
                if (!window.isEventBound) {
                    ...
                    channel.bind('pusher:subscription_succeeded', function(members) {
                        console.log('Users currently in channel:' + members.length);
                        // members.each(function(member) {
                        //     console.log(member.id); // Log each user's ID
                        // });
                    });
                    ...
                    channel.bind('pusher:member_added', function(member) {
                        ...
                        alert('A user joined: ' + member.id); // Optional alert for joining
                    });

                    // Bind to the event for when a user leaves the channel
                    channel.bind('pusher:member_removed', function(member) {
                        console.log('User left:', member.id);
                        alert('A user left: ' + member.id); // Optional alert for leaving
                    });

                    channel.bind('PresenceTestEvent', function(data) {
                        console.log('Received event data:', data);
                        alert(JSON.stringify(data));
                        if (data && data.message) {
                            alert("Received message: " + data.message); // Show message if it exists
